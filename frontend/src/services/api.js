...
export const usersAPI = {
  ...
  unfollow: (id, followerId) =>
    api.delete(`/users/${id}/follow`, { data: { followerId } }),
...
export const relationshipsAPI = {
  follow: (followerId, followeeId) =>
    api.post("/relationships/follow", { followerId, followeeId }),
  unfollow: (followerId, followeeId) =>
    api.post("/relationships/unfollow", { followerId, followeeId }),
  getFollowers: (userId) => api.get(`/relationships/followers/${userId}`),
  getFollowing: (userId) => api.get(`/relationships/following/${userId}`),
  checkFollowStatus: (followerId, followeeId) =>
    api.get(`/relationships/check/${followerId}/${followeeId}`),
  getMutualFriends: (userId1, userId2) =>
    api.get(`/relationships/mutual/${userId1}/${userId2}`),
...
export const checkFollowStatus = async (followerId, followeeId) => {
  const response = await relationshipsAPI.checkFollowStatus(
    followerId,
    followeeId
  );
  return response.data;
