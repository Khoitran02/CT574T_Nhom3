...
async function testMongoDBConnection() {
  try {
    ...
    try {
      ...
      console.log('Sharding not available (single MongoDB instance):', error.message);
    }
    ...
    try {
      ...
      for (const collName of collections) {
        try {
          ...
          if (err.code === 26) {
            console.log(`Collection '${collName}' not found (will be created when first used)`);
          } else {
            console.log(`Collection '${collName}' not sharded yet`);
          }
        }
      }
    } catch (error) {
      console.log('Sharding commands not available');
    }
    ...
    if (serverStatus.sharding) {
      ...
      console.log('Direct mongod connection - not cluster setup');
    }
