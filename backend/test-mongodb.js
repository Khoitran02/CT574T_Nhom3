...
async function testMongoDBConnection() {
  try {
    ...
    try {
      ...
      console.log(
        "✅",
        pingResult.ok === 1 ? "MongoDB connection successful!" : "Ping failed"
      );
    } catch (error) {
    ...
    try {
      ...
      console.log(
        "✅ Sharding enabled with",
        shardStatus.shards.length,
        "shards:"
      );
      ...
      console.log(
        "⚠️  Sharding not available (single MongoDB instance):",
        error.message
      );
    }
    ...
    try {
      ...
      for (const collName of collections) {
        try {
          const shardInfo = await db.runCommand({
            getShardDistribution: collName,
          });
          console.log(`✅ Collection '${collName}' is sharded`);
        } catch (err) {
          if (err.code === 26) {
            console.log(
              `⚠️  Collection '${collName}' not found (will be created when first used)`
            );
          } else {
    ...
    // Summary
    console.log("\n📊 Connection Summary:");
    console.log("   MongoDB URI: mongodb://localhost:27016/socialnetwork");
    console.log("   Database: socialnetwork");
    console.log(
      "   Connection type:",
      serverStatus.sharding ? "Sharded Cluster (mongos)" : "Single Instance"
    );
  } catch (error) {
    ...
    console.log("- Kiểm tra port 27016 có sẵn:");
    console.log("  mongosh --port 27016 --eval \"db.runCommand('ping')\"");
