...
async function testAllDatabaseConnections() {
  try {
    ...
    // Đánh giá tổng thể
    const successfulConnections = testResults.filter(
    ...
    console.log("\n🎯 Final Assessment:");
    console.log(
      `   ✅ Successful: ${successfulConnections.length}/${testResults.length} databases`
    );
    ...
    if (failedConnections.length > 0) {
      ...
      failedConnections.forEach((failed) => {
        if (failed.database === "MongoDB") {
          console.log("   MongoDB:");
          console.log(
            "   - Check if MongoDB cluster is running: Get-Process mongod,mongos"
          );
          console.log(
            "   - Start cluster: .\\script\\start-mongodb-cluster.ps1"
          );
          console.log(
            "   - Check port 27016: mongosh --port 27016 --eval \"db.runCommand('ping')\""
          );
        }
