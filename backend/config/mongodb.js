...
export const connectMongoDB = async () => {
  try {
    const mongoUri = process.env.MONGO_URI || "mongodb://localhost:27016";
...
const testShardingStatus = async (connection) => {
  try {
    ...
    if (admin) {
      ...
      console.log(
        `Sharding active: ${shardStatus.shards.length} shards detected`
      );
    }
  } catch (error) {
    console.warn(
      "Sharding status check failed (may be single MongoDB):",
      error.message
    );
  }
};
...
export const getMongoConnection = () => {
  if (!mongoConnection) {
    throw new Error(
      "MongoDB connection chưa được khởi tạo. Gọi connectMongoDB() trước."
    );
  }
  return mongoConnection;
};
...
export const testMongoConnection = async () => {
  try {
    ...
    try {
      ...
      console.log(
        `MongoDB Sharding: ${shardStatus.shards.length} shards active`
      );
      return {
        status: "connected",
        type: "sharded",
        shards: shardStatus.shards.length,
        uri: process.env.MONGO_URI || "mongodb://localhost:27016",
      };
    } catch (error) {
      return {
        status: "connected",
  ...
  isCollectionSharded: async (collectionName) => {
    try {
      ...
      const shardInfo = await db.runCommand({
        getShardDistribution: collectionName,
      });
      return true;
    } catch (error) {
      return false;
